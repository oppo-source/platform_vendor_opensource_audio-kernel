				...
				u32 *val, unsigned int length)
{
	...
	if (swrm->bulk_write)
		 ...
		 */
			...
			// Kaijia.Lin@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/04/15, Modify for CR#2648163 fixing fifo
...
static void swrm_enable_slave_irq(struct swr_mstr_ctrl *swrm)
{
	...
	/*Liang.Huang@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/08/25, CR2757832 CR2749607 CR2707946 CR2707944 CR2703864 CR2686706 add for the headset will not recognize*/
	u32 temp;
	#endif /* OPLUS_BUG_STABILITY */
	...
	for (i = 0; i < (swrm->master.num_dev + 1); i++) {
		#ifndef OPLUS_BUG_STABILITY
		// Kaijia.Lin@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/03/16, Modify for CR#2627417 fixing headset detection issue
...
static int swrm_master_init(struct swr_mstr_ctrl *swrm)
{
	...
	#ifndef OPLUS_BUG_STABILITY
	/*Liang.Huang@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/08/19, CR27079460 CR2707946 CR2703864 CR2686706 CR2749607 add for the headset will not recognize*/
	reg[len] = SWRM_COMP_CFG_ADDR;
	value[len++] = 0x03;
	#endif /* OPLUS_BUG_STABILITY */
...
static int swrm_probe(struct platform_device *pdev)
{
	...
	#ifdef OPLUS_BUG_STABILITY
	//Nan.Zhong@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/12/21, Add for Fix headset detect issue during clk stop wakeup
	swrm->enable_slave_irq = false;
	swrm->clk_stop_wakeup = false;
	#endif /* OPLUS_BUG_STABILITY */
...
static int swrm_runtime_resume(struct device *dev)
{
	...
	struct swr_device *swr_dev;
	#ifdef OPLUS_BUG_STABILITY
	//Nan.Zhong@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/12/21, Add for Disable clock gating when soundwire is out of clock stop mode
	u32 temp = 0;
	#endif /* OPLUS_BUG_STABILITY */
...
static int swrm_runtime_suspend(struct device *dev)
{
	...
	dev_dbg(dev, "%s: pm_runtime: suspend state: %d\n",
		__func__, swrm->state);
	#ifdef OPLUS_BUG_STABILITY
	//Nan.Zhong@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/10/23, Add for adsp SSR raise wcd fail
