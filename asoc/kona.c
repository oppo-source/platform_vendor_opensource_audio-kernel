...
/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/01/06,
...
/*Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet.1226699, 2017/03/06,
...
static struct afe_clk_set mi2s_mclk[MI2S_MAX] = {
	{
        ...
        /*Chunyu.xie@PSW.MM.AudioDriver.Machine, 2020/07/13, Add for lito config*/
...
#ifdef OPLUS_BUG_COMPATIBILITY
// Kaijia.Lin@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/01/20, Add for BOB noise
...
static struct wcd_mbhc_config wcd_mbhc_cfg = {
	...
	#ifndef OPLUS_ARCH_EXTENDS
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet, 2019/08/31, Modify for disable moisture detect. bit2~3*/
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (++mi2s_intf_conf[index].ref_cnt == 1) {
		...
		#ifdef OPLUS_ARCH_EXTENDS
		/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.CODEC, 2019/07/15,
		 *Add for AKM use as master mode.
		 */
		if ((!mi2s_intf_conf[index].msm_is_mi2s_master)
		...
		if (*dmic_gpio_cnt == 1) {
			...
			if (ret < 0) {
				pr_err("%s: gpio set cannot be activated %sd",
					__func__, "dmic_gpio");
				#ifndef OPLUS_FEATURE_PLATFORM_LITO
				/*Jianfeng.Qiu@PSW.MM.AudioDriver.Platform, 2019/07/15,
				 *Delete for not return when set pin state fail.
				 */
				return ret;
				#endif
			}
...
static int msm_int_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
...
#ifndef OPLUS_FEATURE_PLATFORM_LITO
        /*Chunyu.Xie@MULTIMEDIA.AUDIODRIVER.MACHINE 2020/8/8 Add for fixing compile issue on
        lito platform*/
	...
	#else /*OPLUS_ARCH_EXTENDS*/
	#ifndef OPLUS_FEATURE_PLATFORM_LITO
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.Machine, 2019/07/10, Modify for null pointer*/
	if (rtd->card->num_aux_devs &&
	    !list_empty(&rtd->card->component_dev_list)) {
		list_for_each_entry(aux_comp,
				&rtd->card->aux_comp_list,
...
static struct snd_soc_dai_link msm_wsa_cdc_dma_be_dai_links[] = {
	/* WSA CDC DMA Backend DAI Links */
	{
		...
		.dpcm_playback = 1,
		#ifndef OPLUS_FEATURE_PLATFORM_LITO
		/*Guowang.Huang@PSW.MM.AudioDriver.Machine, 2020/06/22, Delete for no use wsa*/
...
static struct snd_soc_dai_link msm_va_cdc_dma_be_dai_links[] = {
	{
		...
		.dpcm_capture = 1,
		#ifdef OPLUS_FEATURE_PLATFORM_LITO
		/*Guowang.Huang@PSW.MM.AudioDriver.Machine, 2020/06/22, Add for audrx init */
		.init = &msm_int_audrx_init,
		#endif /* OPLUS_FEATURE_PLATFORM_LITO */
...
static struct snd_soc_card *populate_snd_card_dailinks(struct device *dev)
{
	...
	u32 wcn_btfm_intf = 0;
	#ifdef OPLUS_ARCH_EXTENDS
	/*Xiaoke.Zhi@MULTIMEDIA.AUDIODRIVER.CODEC, 2019/06/14, Modify for Max98928*/
	#ifdef OPLUS_FEATURE_PLATFORM_LITO
	/*Guowang.Huang@PSW.MM.AudioDriver.Machine, 2020/06/22, Add for wsa*/
	...
	if (!strcmp(match->data, "codec")) {
		...
		#ifndef OPLUS_FEATURE_PLATFORM_LITO
		/*Guowang.Huang@PSW.MM.AudioDriver.Machine, 2020/06/22, Modify for wsa*/
		memcpy(msm_kona_dai_links + total_links,
