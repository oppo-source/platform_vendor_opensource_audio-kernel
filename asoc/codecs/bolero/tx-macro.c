...
//Yongzhi.Zhang@MULTIMEDIA.AUDIODRIVER.STABILITY, 2020/04/15, Add for CR#2629541 fixing MBHC issue
...
#ifndef OPLUS_BUG_STABILITY
/*Nan.Zhong@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/10/22,Modify for hpf delay fix pop noise*/
#define TX_MACRO_AMIC_HPF_DELAY_MS	300
#else /* OPLUS_BUG_STABILITY */
#define TX_MACRO_AMIC_HPF_DELAY_MS	500
#endif /* OPLUS_BUG_STABILITY */
#ifdef OPLUS_BUG_STABILITY
// Yongzhi.Zhang@MULTIMEDIA.AUDIODRIVER.CODEC, 2020/04/16, add for CR#2629541 fixing MBHC issue
 ...
 *
 * @tx_priv: pointer to tx macro priv resource
 * @o: pm old state
 * @n: pm new state
 *
 * Returns old state
...
static bool tx_macro_lock_sleep(struct tx_macro_priv *tx_priv)
{
	...
	/*
	 * tx_macro_{lock/unlock}_sleep will be called by tx macro irq handler
...
static int tx_macro_suspend(struct device *dev)
{
	...
	if (tx_priv->pm_state == TX_MACRO_PM_SLEEPABLE) {
		...
		/*
		 * unlock to wait for pm_state == TX_MACRO_PM_SLEEPABLE
		 * then set to TX_MACRO_PM_ASLEEP
		 */
