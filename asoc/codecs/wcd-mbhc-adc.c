...
/* Zhao.Pan@MULTIMEDIA.AUDIODRIVER.HEADSETDET, 2020/08/28,
...
/* Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet, 2018/12/06,
...
static int wcd_mbhc_adc_get_spl_hs_thres(struct wcd_mbhc *mbhc)
{
	...
	#ifdef OPLUS_ARCH_EXTENDS
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet, 2019/09/21, Add for log*/
...
#define pr_debug pr_info
	...
	while (!is_spl_hs) {
		...
		/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.HeadsetDet, 2019/09/25, Modify for adc measure failed*/
...
#define pr_debug pr_info
	...
	if (mbhc->mbhc_cb->mbhc_micbias_control) {
		mbhc->mbhc_cb->mbhc_micbias_control(component, MIC_BIAS_2,
						    MICB_ENABLE);
		#ifdef OPLUS_ARCH_EXTENDS
		/* Wei.Hua@MULTIMEDIA.AUDIODRIVER.HeadsetDet, 2020/12/3, workaround to fix headset pop noise*/
...
#ifdef OPLUS_ARCH_EXTENDS
/* Le.Li@MULTIMEDIA.AUDIODRIVER.HeadsetDet, 2018/03/22, Add for headset detect */
...

{
	...
	#ifdef OPLUS_ARCH_EXTENDS
	/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.HeadsetDet, 2019/09/25, Add for invalid adc result*/
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	#endif
#ifdef OPLUS_ARCH_EXTENDS
/* Zhao.Pan@PSW.MM.AudioDriver.HeadsetDet, 2018/12/06, Add for log*/
#undef pr_debug
#define pr_debug pr_info
#endif /* OPLUS_ARCH_EXTENDS */
	...
	while (!time_after(jiffies, timeout)) {
		...
		if (output_mv > hs_threshold) {
			...
			if (plug_type != MBHC_PLUG_TYPE_GND_MIC_SWAP) {
				...
				#ifdef OPLUS_ARCH_EXTENDS
				/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.HeadsetDet, 2019/09/25, Add for quick report 3pole*/
				if ((mbhc->current_plug == MBHC_PLUG_TYPE_HEADSET)
					&& (!wcd_swch_level_remove(mbhc))) {
...
report:
	...
	#ifdef OPLUS_ARCH_EXTENDS
	/*Jianfeng.Qiu@MULTIMEDIA.AUDIODRIVER.HeadsetDet.1042404, 2019/09/25, Add for remove report*/
